...
older than some other packs want — if other workflows break after installing ReDetail, that's why.
...
Both output dimensions must divide by **64**, not 32 — the IC-LoRA guide splits the latent into 2x2
...
stays /64. The CLI works all this out, and also puts your *source* on the grid — it may crop a few
...
ComfyUI. It needs a directly reachable, unauthenticated endpoint — it sends no credentials. Pass
